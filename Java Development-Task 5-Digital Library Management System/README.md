...
A new administrator can register from `/register.html` by selecting **Administrator** as the account type.
...
Keep this code secret and do not commit it to GitHub.

Regular users can register without the admin registration code.
...
The server validates the admin registration code before creating an administrator account.
